...
#ctrl+shift+p, select interpreter, choose the streamlitenv.

#streamlit hello
#streamlit run your_app.py
#pip install streamlit pandas pathlib

#카테고리 수정, #프롬프트 작성 글에 구체적인 형식 지정, #데이터 관리.
#작성하고 삭제할 수 있는지 확인, ...#프롬프트 내용설명할 수 있는 것도 넣자
#프롬프트 원론에 대한 글도 넣자
#url 하나 사고 갤러리 업데이트하고 겁나 큰 장으로 만들자
#여기에 그것도 넣자 로펌의 선업글 하는 것
#전반적으로 수정하기 
#폼형태 수정, 디자인 수정
#활용사례 대댓글이 밑에 쭉 달리는 것
#그걸 보고 인기 대댓글도!
#회원별 등급도 나누기
#프롬프트 복사하는 기능
...
# from 프롬프트갤러리1 import update_prompt

# # 설정 및 페이지 구성
# st.set_page_config(
#     page_title="법률 AI 프롬프트 갤러리",
#     page_icon="⚖️",
#     layout="wide"
# )
...
#     # 초기 프롬프트 데이터 (실제로는 데이터베이스에서 로드)
#     st.session_state.prompts = {
#         "1": {
#             "title": "계약서 검토 프롬프트",
#             "content": "다음 계약서를 검토하고 주요 위험 조항을 분석해주세요:",
#             "category": "계약검토",
#             "author": "변호사A",
#             "date": "2024-03-15",
#             "ratings": [4, 5, 4],
...
# def save_prompt(title, content, category, author):
#     """새로운 프롬프트 저장"""
#     prompt_id = str(len(st.session_state.prompts) + 1)
#     st.session_state.prompts[prompt_id] = {
#         "title": title,
#         "content": content,
#         "category": category,
#         "author": author,
...
# def add_rating(prompt_id, rating):
#     """프롬프트 평점 추가"""
#     st.session_state.prompts[prompt_id]["ratings"].append(rating)

# def toggle_like(prompt_id):
#     """좋아요 토글"""
#     current_likes = st.session_state.prompts[prompt_id]["likes"]
#     st.session_state.prompts[prompt_id]["likes"] = current_likes + 1

# # 사이드바 - 카테고리 필터
# st.sidebar.title("필터")
# categories = ["전체", "계약검토", "법률자문", "소송문서", "기타"]
# selected_category = st.sidebar.selectbox("카테고리 선택", categories)

# # 메인 페이지 제목
# st.title("법률 AI 프롬프트 갤러리 🤖⚖️")

# # 탭 생성
# tab1, tab2 = st.tabs(["프롬프트 둘러보기", "새 프롬프트 작성"])

# # 탭1: 프롬프트 목록 및 검색
# with tab1:
#     # 검색 기능
#     search_term = st.text_input("프롬프트 검색", "")
    
#     # 프롬프트 표시
#     for prompt_id, prompt in st.session_state.prompts.items():
#         if (selected_category == "전체" or prompt["category"] == selected_category) and \
#            (search_term.lower() in prompt["title"].lower() or search_term.lower() in prompt["content"].lower()):
            
#             with st.expander(f"{prompt['title']} - {prompt['author']}"):
#                 st.write(f"**카테고리:** {prompt['category']}")
#                 st.write(f"**작성일:** {prompt['date']}")
#                 st.text_area("프롬프트 내용", prompt['content'], height=100, key=f"content_{prompt_id}")
                
#                 # 평점 및 좋아요
#                 col1, col2 = st.columns([1, 1])
#                 with col1:
#                     avg_rating = sum(prompt['ratings']) / len(prompt['ratings']) if prompt['ratings'] else 0
#                     st.write(f"평균 평점: {'%.1f' % avg_rating} ⭐ ({len(prompt['ratings'])}개 평가)")
#                     new_rating = st.select_slider(
#                         "평가하기",
#                         options=[1, 2, 3, 4, 5],
#                         key=f"rating_{prompt_id}"
#                     )
#                     if st.button("평가 제출", key=f"submit_rating_{prompt_id}"):
#                         add_rating(prompt_id, new_rating)
#                         st.success("평가가 제출되었습니다!")
                
#                 with col2:
#                     st.write(f"좋아요: {prompt['likes']} 👍")
#                     if st.button("좋아요", key=f"like_{prompt_id}"):
#                         toggle_like(prompt_id)
...
#     with st.form("new_prompt_form"):
#         title = st.text_input("제목")
#         content = st.text_area("프롬프트 내용", height=200)
#         category = st.selectbox("카테고리", categories[1:])  # "전체" 제외
#         author = st.text_input("작성자")
        
#         submitted = st.form_submit_button("제출")
#         if submitted:
#             if title and content and category and author:
#                 save_prompt(title, content, category, author)
#                 st.success("새로운 프롬프트가 등록되었습니다!")
#             else:
#                 st.error("모든 필드를 입력해주세요.")

# # 페이지 하단 스타일링
# st.markdown("---")
# st.markdown("### 📚 프롬프트 작성 가이드라인")
# st.markdown("""
# - 명확하고 구체적인 지시사항을 포함해주세요
# - 예시와 함께 작성하면 더 효과적입니다
# - 개인정보가 포함되지 않도록 주의해주세요
...
#                     st.error("모든 필드를 입력해주세요.")
#     else:
#         st.warning("프롬프트를 작성하려면 먼저 로그인해주세요.")
...
#         with st.form("edit_prompt_form"):
#             edited_title = st.text_input("제목", value=prompt[1])
#             edited_content = st.text_area("프롬프트 내용", value=prompt[2], height=200)
#             edited_category = st.selectbox("카테고리", categories[1:], 
#                                          index=categories[1:].index(prompt[3]))
